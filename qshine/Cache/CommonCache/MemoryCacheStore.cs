...
namespace qshine
{
	...
	public class MemoryCacheStore : ICache
	{
		...
		/// If the cached item changed it will automatically remove dependant cached items</param>
		...
		public void Add(string key, object value, DateTime absoluteExpiration, TimeSpan slidingExpiration, IEnumerable<string> dependentSets, CacheItemPriority priority)
		{
			...
			{
				foreach (var d in dependentSets)
				{
					_cache.Remove(d);
				}
			}
