...

{
    ...
    /// </summary>
    public abstract class IoCImplBase :IIoCContainer
    {

		#region fields
		IoCLifetimeScope defaultLifetime = IoC.DefaultLifetimeScope;
        #endregion

        #region IIoCContainer Members

		#region DefaultLifetime

		public virtual IoCLifetimeScope DefaultLifetimeScope
		{
			get
			{
		...
		/// <param name="name">The name of IoC registration for the given requested type</param>
		/// <returns>The actual implementation class instance object to be return</returns>
        public virtual object Resolve(Type requestedType, string name)
        {
        ...
        public virtual IIoCContainer RegisterType<IT, T>(IoCLifetimeScope lifetimeOption, params NamedValue[] constructorParameters)
            where IT : class
            where T : class, IT
        {
            return RegisterType(typeof(IT), typeof(T), string.Empty, lifetimeOption, constructorParameters);
        }

        public virtual IIoCContainer RegisterType<IT, T>(string name,IoCLifetimeScope lifetimeOption, params NamedValue[] constructorParameters)
            where IT : class
            where T : class, IT
        {
            return RegisterType(typeof(IT), typeof(T), name, lifetimeOption, constructorParameters);
        }
        ...
        public virtual IIoCContainer RegisterType<IT>(Type actualType, IoCLifetimeScope lifetimeOption, params NamedValue[] constructorParameters)
            where IT : class
        {
            return RegisterType(typeof(IT), actualType, string.Empty, lifetimeOption, constructorParameters);
        }

        public virtual IIoCContainer RegisterType<IT>(Type actualType, string name, IoCLifetimeScope lifetimeOption, params NamedValue[] constructorParameters)
            where IT : class
        {
            return RegisterType(typeof(IT), actualType, name, lifetimeOption, constructorParameters);
        }
        ...
        {
            return RegisterType(requestedType, actualType, name, DefaultLifetimeScope, constructorParameters);
        }

        public virtual IIoCContainer RegisterType(Type requestedType, Type actualType, IoCLifetimeScope lifetimeOption, params NamedValue[] constructorParameters)
        {
            return RegisterType(requestedType, actualType, string.Empty, lifetimeOption, constructorParameters);
        }
        ...
        /// <param name="lifetimeOption">Option to manage the life time of instance creating.</param>
        /// <returns>Current container instance</returns>
        public virtual IIoCContainer RegisterType(Type requestedType, Type actualType, string name, IoCLifetimeScope lifetimeOption, params NamedValue[] constructorParameters)
        {
            throw new NotImplementedException();
        }
        ...
        public virtual IIoCContainer RegisterModule(object module)
        {
            ...
            catch (Exception ex)
            {
                throw new IoCException(
                    string.Format(Resource.IoCExceptionRegisterModuleMessage),
                    ex);
            }
            return this;
        }
        ...
        protected IoCLifetimeScope GetLifetimeScope(string lifetimeOption)
        {
            IoCLifetimeScope result;
            //Force life time scope option to be Singleton when the web environment is unavaliable.
			if (lifetimeOption == "HttpRequest" && !EnvironmentEx.IsWebApplication)
            {
                lifetimeOption = "Transient";
            }
            ...
            switch (lifetimeOption)
            {
                case "Singleton":
                case "":
                    result = IoCLifetimeScope.Singleton;
                    break;
                case "Transient":
                    result = IoCLifetimeScope.Transient;
                    break;
                case "HttpRequest":
                    result = IoCLifetimeScope.HttpRequest;
                    break;
                default:
                    throw new NotImplementedException();
            }
